 ...
 */
import { PHYS, zonePower } from '../engine/index.js';
import { sel, byId, changed, retraceQuiet, cosmetic, removeComp } from './state.js';
import { esc } from './util.js';

function drawProfile(container, c) {
  const el = container.querySelector('#profPlot'); if (!el) return;
  const a = Object.assign({ design: PHYS[c.type].design }, c.p);
  const W = 214, Hh = 86, n = 240, sd = c.p.sd;
  let pmin = 1e9, pmax = -1e9; const pts = [];
  for (let i = 0; i <= n; i++) { const h = -sd + 2 * sd * i / n, P = zonePower(a, h); pts.push([h, P]); pmin = Math.min(pmin, P); pmax = Math.max(pmax, P); }
  if (pmax - pmin < 0.5) { pmax += 0.25; pmin -= 0.25; }
  const pad = 0.12 * (pmax - pmin); pmin -= pad; pmax += pad;
  const X = h => ((h + sd) / (2 * sd)) * (W - 30) + 26, Y = P => Hh - 12 - ((P - pmin) / (pmax - pmin)) * (Hh - 22);
  let path = ''; pts.forEach(([h, P], i) => { path += (i ? 'L' : 'M') + X(h).toFixed(1) + ',' + Y(P).toFixed(1); });
  const zero = (0 > pmin && 0 < pmax) ? `<line x1="26" y1="${Y(0).toFixed(1)}" x2="${W - 4}" y2="${Y(0).toFixed(1)}" stroke="#2a3441" stroke-dasharray="3 3"/>` : '';
...
export function createPropsPanel(container) {
  function render() {
    ...
    const c = byId(sel), d = PHYS[c.type];
    box.className = '';
    let html = `
    ...
    if (d?.src) html += `<div class="field"><label>Ray color</label><input type="color" id="pColor" value="${c.p.color}"></div>`;
    if (d?.act === 'zone') html += `<div class="field"><label>Power profile P(h) — the design</label><div id="profPlot"></div></div>`;
    ...
    box.querySelector('#pLabel').oninput = e => { c.label = e.target.value; cosmetic(); };
    box.querySelector('#pX').onchange = e => { c.x = +e.target.value; changed(); };
    box.querySelector('#pY').onchange = e => { c.y = +e.target.value; changed(); };
    ...
    box.querySelectorAll('.phys').forEach(inp => {
      inp.oninput = e => {
        const k = e.target.dataset.k; c.p[k] = +e.target.value;
        box.querySelector('#v_' + k).textContent = c.p[k];
        retraceQuiet();
        if (PHYS[c.type]?.act === 'zone') drawProfile(box, c);
      };
    });
    const pc = box.querySelector('#pColor'); if (pc) pc.oninput = e => { c.p.color = e.target.value; retraceQuiet(); };
    box.querySelector('#pVis').onchange = e => { c.visible = e.target.checked; changed(); };
