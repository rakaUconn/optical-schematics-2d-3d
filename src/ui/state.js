...
export function setShowLabels(v) {
  showLabels = v;
}
export function setBackward(v) {
  backward = v;
  setBackwardPropagation(v);
  retraceQuiet();
}
...
export function markDirty3dClean() {
  dirty3d = false;
}
...
export function addComp(type) {
  const parent =
    sel != null ? byId(sel) : state.comps[state.comps.length - 1] || null;
  let x = 0,
    y = 0,
    ang = 0;
  if (parent) {
    const th = (parent.angle * Math.PI) / 180;
    x = parent.x + Math.cos(th) * 150;
    y = parent.y + Math.sin(th) * 150;
    ang = parent.angle;
  }
  const c = {
    id: state.nextId++,
...
export function copyComp(id) {
  const comp = byId(id);
  if (comp) {
    clipboard = JSON.parse(JSON.stringify(comp));
  }
}

export function pasteComp() {
  if (clipboard) {
    const newComp = JSON.parse(JSON.stringify(clipboard));
    newComp.id = state.nextId++;
    newComp.x += 20;
    newComp.y += 20;
...
export function loadState(newState) {
  ...
  comps.forEach((c) => {
    c.p = Object.assign(defaultsFor(c.type), c.p || {});
  });
  state.comps = comps;
  state.nextId =
    newState.nextId ?? comps.reduce((m, c) => Math.max(m, c.id), 0) + 1;
  sel = null;
  changed();
}
